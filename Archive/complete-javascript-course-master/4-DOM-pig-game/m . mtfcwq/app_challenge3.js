...
|                                                                 |                             | Add second dice                                                 |
|                                                                 |                             | Player can play with 2 dice                                     |
|                                                                 |                             | If either dice is a '1' lose current score                      |
|                                                                 |                             | If either of the dice is a '6' 2 times in a row, lose all score |
...
document.querySelector('.btn-roll').addEventListener('click', function() { 
    if(gamePlaying) {
        ...
        var dice_1 = Math.floor(Math.random() * 6) + 1; //Math.random gives a random between 0 and 1; multiplying by 6 => random between 0 and 5; Math.floor => skip decimals
        var dice_2 = Math.floor(Math.random() * 6) + 1;
        var twoDiceSummedScore = dice_1 + dice_2;
        ...
        document.querySelector('.dice-2').src = 'dice-' + dice_2 + '.png';

        if (dice_1 === 6 || dice_2 === 6 && previousDice1 === 6 || previousDice2 === 6) {
            ...
            //Add score 
            roundScore = roundScore + twoDiceSummedScore;
            document.querySelector('#current-' + activePlayer).textContent = roundScore;          
        } else {
            nextPlayer();
        }
        previousDice1 = dice_1;
        previousDice2 = dice_2;
    }
});
...
function init() {
    ...
    document.querySelector('.dice-1').style.display = 'none'; //changes value of a css element
    document.querySelector('.dice-2').style.display = 'none';
